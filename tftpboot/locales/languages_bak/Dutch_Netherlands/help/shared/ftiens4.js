...
if(this.getID()!=foldersTree.getID()){
 ...
 // docW = docW + "<tr>" + leftSide + "<td valign=middle>";
  if (USEICONS)
  {
    ...
    docW = docW + "<img id='folderIcon" + this.id + "' name='folderIcon" + this.id + "' src='" + this.iconImageSrc() + "' alt='onderwerp uitvouwen/samenvouwen' border=0></a>"
  }
  else
  {
	  if (this.prependHTML == "")
...
{ 
  ...
  { 
    ...
    if (isTextLink) {
	if (USEFRAMES)
	  docW = docW + "<a href='" + this.hreference + "' TITLE='"+this.desc+"' TARGET=\"basefrm\" "
	else
	  docW = docW + "<a href='" + this.hreference + "' TARGET=_top "
    }
    else
	docW = docW + "<a href='javascript:void(0)' TARGET=\"basefrm\" "
...
//      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2lastnode.gif' width=16 height=22></td>"
 ...
 //leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2blank.gif' width=16 height=22></td>"
  ...
  if (USEICONS)
      docW = docW + "<a " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' alt='onderwerp openen' border=0>" + "</a>"
  else
	  if (this.prependHTML == "")
...
{ 
  ...
  {
    alert("Treeview kan het node-object voor ID=" + folderId + " niet vinden. Als het configuratiebestand waarden a.xID bevat, moeten deze worden ingesteld voor ALLE nodes, inclusief de root foldersTree.")
    return;
  }
...
{ 
  ...
  {
    alert("Treeview kan het node-object voor ID=" + folderId + " niet vinden. Als het configuratiebestand waarden a.xID bevat, moeten ook foldersTree.xID worden ingesteld.")
    return;
  }
...
{ 
  if (optionFlags>=0) { //is numeric (old style) or empty (error)
    ...
    alert("Wijzig het Treeview-configuratiebestand zodat het nieuwe gegevenstype wordt gebruikt voor het doelargument in gLnk.");
    return;
  }
...
function getElById(idVal) {
  ...
  alert("Probleem bij het ophalen van een element op basis van een ID")
  return null
}
      ...
      if (fldStr != "") {
        ...
        if (nodeObj!=null) //may have been deleted
          if (nodeObj.setState) {
            ...
            alert("Interne ID verwijst niet meer naar een map.\nU kunt dit oplossen door een ID toe te wijzen aan de boomstructuur en externe ID's toe te wijzen aan afzonderlijke nodes.")
      }
    }
  }
}
